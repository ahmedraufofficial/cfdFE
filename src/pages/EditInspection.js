    ...
    const fetchEditData = (id, type) => {
        fetch(`${process.env.REACT_APP_API}/${type}/${id}`)
          ...
          .then(data => {
            ...
            console.log(`${process.env.REACT_APP_API}/${type}/${id}`)
            if (type == "appointment"){
                try{
                    inspection['Vehicle_Manufacturer'] = inspection.Car_Valuation_Details.Make
                    delete inspection.Car_Valuation_Details.Make
                } catch(err) {
                    console.log(err)
                }
                try{
                    inspection['Model'] = inspection.Car_Valuation_Details.Model_Name
                    delete inspection.Car_Valuation_Details.Model
                } catch(err) {
                    console.log(err)
                }
                try{
                    inspection['Manufacturing_Year'] = inspection.Car_Valuation_Details.Model_Year
                    delete inspection.Car_Valuation_Details.Model_Year
                } catch(err) {
                    console.log(err)
                }
                try{
            ...
            <Paper elevation={10} style={paperStyle}>
                <FormikStepper
                    ...
                    onSubmit={(values) => {                  
                        try {
                            async function Edit(x) {
                                if (location.pathname.includes("add") && type == "inspection") {
                                    ...
                                    if (data) {
                                        if (data.status === '200')
                                        {
                                            await fetch(`${process.env.REACT_APP_API}/appointment/${location.pathname.split("/")[3]}`, {
                                                method: 'DELETE',
                                            })
                                    ...
                                    const response = await fetch(`${process.env.REACT_APP_API}/edit/inspection/${edit._id}`, {
                                        method: 'PUT',
                                        headers: {'Content-Type': 'application/json'},
                                        body: JSON.stringify({
                                            x
                    ...
                    <FormikStep key={3} label="Car Exterior">
                        ...
                        <CustomRadioCheckboxField group="Car_Exterior.Rims" name="Rear_Right" condition={["Good", "Average", "Damaged", null]} label={["Faded", "Scratched", "Cracked", "May require replacement"]} />
                    ...
                    <FormikStep key={5} label="Technical Condition" labelColor="#FFFFFF" circleColor="#003399"> 
                        ...
                        <CustomRadioCheckboxField group="Technical_Condition" name="Transmission_Condition" condition={["Good", "Average", "Damaged", null]} label={["Rough shifting", "Delay shifting", "Jerking", "Slipping", "Not shifting", "Triptonic faulty", "Clutch faulty", "Noisy", "May require maintainence"]} />

                        <CustomRadioCheckboxField group="Technical_Condition" name="Sign_Of_Leakages" condition={["None", "Yes (Minor)", "Yes (Major)", null]} label={["Gaskit", "Head gaskit", "Engine oil seals", "Engine oil sump", "Differntial leak", "Power steering oil leak", "Brake oil leak", "Axle boot leak", "Fuel leak", "Coolant leak", "May require maintainence"]} />
                    ...
                    <FormikStep key={6} label="Customer and Appointment Information">

                        <Grid><h3>Customer and Appointment Information</h3></Grid>
